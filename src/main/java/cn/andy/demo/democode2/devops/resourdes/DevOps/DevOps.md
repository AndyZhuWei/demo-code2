...
- 拉取Jenkins镜像
  ...
  实验时使用了
```shell
docker pull jenkins/jenkins:2.332.1-lts
```
在192.168.80.103机器的/usr/local/docker/jenkins_docker下新建docker-compose.yml文件，内容如下
...
- 编写docker-compose.yml
  ...
  services:
    jenkins:
      image: jenkins/jenkins  #替换成jenkins/jenkins:2.332.1-lts
...
- 首次启动会因为数据卷data目录没有权限导致启动失败，设置data目录写权限
  ...
  chmod -R 777 data/
  ```
  然后重新启动，在/usr/local/docker/jenkins_docker目录下执行
```shell
